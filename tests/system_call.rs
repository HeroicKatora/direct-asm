...
#[direct_asm::assemble]
unsafe extern "C" 
fn sys_write(fd: c_int, ptr: *const c_void, len: size_t)
    -> ssize_t 
{
    "mov %rax, ${LONG SYS_write}"; // Move sys call number to rax as required
...
fn sys_print(what: &str) -> libc::ssize_t {
    unsafe {
        sys_write(1, what.as_ptr() as *const libc::c_void, what.len())
    }
}
